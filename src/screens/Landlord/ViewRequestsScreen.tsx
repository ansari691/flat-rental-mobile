...
const ViewRequestsScreen = () => {
  ...
  const fetchRequests = async () => {
    ...
    try {
      const reqList = await requestService.getLandlordRequests();
      setRequests(reqList);
    } catch (error) {
      console.error('Error fetching requests:', error);
      Alert.alert('Error', 'Could not load rental requests.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleStatusUpdate = async (requestId: string, status: string) => {
    try {
      await requestService.updateRequestStatus(requestId, status as 'approved' | 'rejected');
      fetchRequests();
    } catch (error) {
    ...
    <View style={styles.container}>
      <View style={styles.headerContainer}>
        <Text style={styles.requestCount}>
          {requests.filter((req) => req.status !== 'denied').length} request
          {requests.filter((req) => req.status !== 'denied').length !== 1
            ? 's'
            : ''}{' '}
          received
        </Text>
